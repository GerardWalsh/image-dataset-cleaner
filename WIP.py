...
from src.utils.hash import convert_hash, dhash
...
# ap.add_argument("-i", "--query_image", required=True, type=str,
# 	help="path to input query image")
ap.add_argument("-q", "--query", required=False, type=str,
	help="path to input query directory")
...
# loop over images in query directory
for (i, imagePath) in enumerate(list(paths.list_images(args["query"]))):
	print(imagePath)
...
# 		len(resultPaths), d, h), resultPaths)
